...
**Current release: v3.6.0** (3 Aug 2026) — the coefficient campaign closes: GRV-088's four-order whisper tension adjudicated in-house as a CATEGORY ERROR (temperature coefficient vs ringing frequency; Campbell's theorem) — the pitch is geometric (0.23 kappa, GRV-040 vindicated), the rate thermodynamic (GRV-087 vindicated), the black hole a crackling-noise emitter. Score: beta promoted (operating point measured), pile-up eliminated (exact cancellation), identification vindicated, confrontation convicted. Residue: the quantum of the ring. See `docs/RELEASE_NOTES_v3.6.0.md` (and v3.3.0–v3.5.0 notes for the ribbon and strong-field campaigns this line previously failed to reflect — README bump had been silently pattern-missing since v3.3.0; caught and fixed this release).
