...
**Current release: v3.9.0** (4 Aug 2026) — the quantum arrives whole: the framework's third founding assumption adopted (indivisible delivery, falsifiers armed, retirement bounty standing), and one sentence buys the beamsplitter (Grangier-Roger and Fock counting), antibunching (with the loss-invariance faithfulness theorem; the QB pin resolved, not contradicted), Mandel counting (with the correspondence theorem: the semiclassical and granular descriptions of classical light are one theory at two grains), and delayed choice as a derived non-paradox (V^2 + D^2 = 1 as energy bookkeeping). The HOM boundary pinned from both internal routes with the fourth-grant escape specified and not purchased; the candidate census held the bet count at one (ELEC-084); the paper library standardized under one attribution; the detector paper published. See `docs/RELEASE_NOTES_v3.9.0.md`.
