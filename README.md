...
**Published:** [https://doi.org/10.5281/zenodo.21430784](https://doi.org/10.5281/zenodo.21430784) · **Author ORCID:** [https://orcid.org/0009-0007-2454-5573](https://orcid.org/0009-0007-2454-5573) · **Version:** 3.14.0
...
>
> **What makes it unusual is the discipline.** Every claim is registered with a pass/fail line drawn *before* the computation runs, and the failures are kept on permanent display rather than quietly dropped. As of this release: **549 registered claims, 525 backed by code you can run yourself, 113 proved from the stated assumptions (Derived), and 32 failures kept in the open.** A dealbreaker, if one exists, should be findable in about five minutes — start with [`KNOWN_LIMITATIONS.md`](KNOWN_LIMITATIONS.md).
>
...
> **Corpus state:** 549 registered claims (113 Derived, 388 Modeled, 4 EFT-constrained, 4 Conjecture, 8 Open, 32 Failed-and-kept); 525 code-backed, all passing; 59 papers. Counts regenerate from `claims.yaml` — the authority is the registry, not this line.
...
**Current release: v3.14.0** (9 Aug 2026) — the unification release. For a stretch the corpus existed as two development lineages that had diverged and could not both be authoritative; this release merges them into one, recovers the results lost in the split, and closes the zero-point lever cycle. Seven review-arc commissions whose registration texts were lost with an intermediate packaging were recovered by **re-derivation with contamination disclosed** (results read, blind bars unavailable, every recorded number confronted and reproduced to 0–1%): the electron-energy circularity theorem, the three-pin scale problem, the knot-stability spectrum, the star-mode spectrum, the **dispersion coefficient** (β = 1/12 − B/(T0 a²), the M-point mesh excluded in the photon sector by ~11 orders plus a band-top existence kill, the LHAASO GRB 221009A bound verified live), the **loaded-continuum rescue** (killed twice — a 53-order photon-mass deficit and a g-independent bending superluminality), and the **torsion carrier** (killed on state-count, the carrier channel-map closed). Alongside the merge, the matter sector's **zero-point lever** ran a full life cycle and then a correction: the successor mechanism (contact-modified dispersion) was derived blind and died on three-census exclusivity (FND-MATTER-063), after which a provenance audit found the lever's long-cited "25 percent" was never a measured quantity — it was the inter-knot fraction spread of an old benchmark, drifted across a claim boundary (FND-MATTER-064) — dissolving the derived target and reverting λ to an honest prediction slot, with π(r/a)² adopted by grant as the standing lever prediction and its falsifier armed (FND-MATTER-065). This release sits on top of the **alpha summit** of the prior line (1/α = 4 π³ · D_E = 137.060504, +178.8 ppm, every factor derived, g = 2 out of sample; **NOT a derivation of α's value** — a reduction to one blind number times a derived prefactor) and the nuclear 13%→1% closure (NUC-A..D2, complete by theorem), both retained in full. Every re-derived claim carries its contamination-disclosed flag; record-grade sub-items are flagged where nothing downstream leans on them; one benchmark script remains unrecoverable and is disclosed as such. See `docs/history/RELEASE_NOTES_v3.14.0.md` and `docs/commissions/MERGE_MANIFEST.md`.
