...
**Current release: v3.15.0** (10 Aug 2026) — the electromagnetic arc. Five chartered commissions with bars locked before computation rebuilt the electromagnetic sector from registered mechanics and closed the carrier question the channel-map had left as a wall of negatives: **the direction** of E derived from the operational force-per-winding definition (EM-RECON-024, with the necessity clause unifying two prior kills into one fact); **the carrier** identified as the mesh's collective transverse Goldstone pair — exactly gapless by translation symmetry, the earlier mass kill located as a frozen-background artifact in the operator itself (EM-RECON-025, carrying a mandatory coverage-vs-dispersive fork and a live bending falsifier B ≤ T0a²/12); **the electric sign** derived from the momentum-flux (Blasius) integral with the static sign ledger reproduced from momentum conservation and the full Lorentz structure emerging as one Magnus formula read twice (EM-RECON-026); **the normalization** κ₀ = c/√(ε₀Σ) fixed identically by two independent routes with Coulomb emerging residue-free — no new constant exists, everything gates on the bridge's Σ (EM-RECON-027); and **the magnetic sector reconciled** — the ether-drag worry cancelling by winding closure (∮dr = 0, exhibited on a trefoil), μ₀ = 1/(ε₀c²) forced by the carrier's own dispersion, and the registered magnetism claims landed on as independent confirmations (EM-RECON-028). The arc ends with its neck out: **Predictions 28–31** (the subluminal sign lock; the √3-locked dispersion/transparency fingerprint; the ≥2×10¹¹ GeV massive partner; the one-number Σ lock with its quantified vacuum-density obligation), each with its kill condition on its face (EM-RECON-029). Zero free parameters introduced across the arc. This release sits on v3.14.0's unification (the two-lineage merge and seven-result recovery, retained in full). See `docs/history/RELEASE_NOTES_v3.15.0.md`.
