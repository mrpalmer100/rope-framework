"""ROPE-VALIDATION-004: curved closed-core 3-D Aharonov--Bohm validation.

Instrument validation only. A circular closed flux core is excluded from a
Cartesian 3-D domain. Outside the core, the lattice connection is flat and its
only physical content is an externally imposed meridional holonomy around the
closed core. Distributed-link and twisted-cut gauges are compared.

This does not claim that a rope supplies or dynamically generates flux.
"""
...
def make_domain(n, r_core):
    ...
    rho = np.sqrt(X*X + Y*Y)
    d = np.sqrt((rho-LOOP_RADIUS)**2 + Z*Z)
    mask = d > r_core
...
def core_phase(xyz):
    ...
    # Meridional angle around the circular excluded core.
    return np.arctan2(xyz[:,2], rho-LOOP_RADIUS)


def build_hamiltonian(n, r_core, alpha, gauge='distributed', chi=None):
    x, h, xyz, ijk, lookup = make_domain(n, r_core)
    N = len(xyz)
    theta = core_phase(xyz)
    rho = np.sqrt(xyz[:,0]**2 + xyz[:,1]**2)
    d = np.sqrt((rho-LOOP_RADIUS)**2 + xyz[:,2]**2)
    tor = np.arctan2(xyz[:,1], xyz[:,0])
    # Shell confinement around the excluded circular core, with weak toroidal
    # pinning to keep the lowest states compact in all three dimensions.
    V = KMER*(d-SHELL_RADIUS)**2 + KTOR*(1.0-np.cos(tor))
...
def spectrum(H, k=N_EIG):
    vals = eigsh(H, k=k, which='SA', tol=2e-9, maxiter=15000,
...
def run():
        ...
        for n in LEVELS:
            for alpha in ALPHAS:
                ...
                if alpha == 0.25 and n == LEVELS[-1]:
                    num=np.linalg.norm((H-H.getH()).data)
                    den=max(np.linalg.norm(H.data),1e-30)
                    herm.append(float(num/den))
    ...
    bars={
      ...
      'B6_nontrivial_half_flux_response': min_response > 0.03,
      'B7_grid_convergence': max_grid < 0.12,
    }
    finding='AB_3D_CURVED_EXCLUDED_CORE_INSTRUMENT_VALIDATED' if all(bars.values()) else 'AB_3D_CURVED_EXCLUDED_CORE_VALIDATION_INCOMPLETE'
    summary={
      'scope':'3-D curved closed excluded-core AB solver validation only; holonomy is imposed externally and not derived from rope dynamics',
      'box':BOX,'levels':LEVELS,'core_radii':CORE_RADII,'alphas':ALPHAS,
      'loop_radius':LOOP_RADIUS,'shell_radius':SHELL_RADIUS,'n_eig':N_EIG,
      ...
      'technical_note':'The circular core is excluded from the wavefunction domain. The multivalued meridional phase defines a flat lattice connection on the accessible complement, with externally imposed holonomy around the closed curved core.'
    }
    out=ROOT/'analysis'; out.mkdir(exist_ok=True)
    with open(out/'ROPE_VALIDATION004_spectrum.csv','w',newline='') as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)
    (out/'ROPE_VALIDATION004_summary.json').write_text(json.dumps(summary,indent=2))
    lines=['ROPE-VALIDATION-004 curved closed excluded-core 3-D Aharonov--Bohm instrument test',summary['scope'],
           f'levels={LEVELS} core radii={CORE_RADII} alphas={ALPHAS} eigs={N_EIG}',
           f'loop radius={LOOP_RADIUS} shell radius={SHELL_RADIUS}',
    ...
    (out/'ROPE_VALIDATION004_run.log').write_text(text+'\n')
    (out/'ROPE_VALIDATION004_results.md').write_text(
      '# ROPE-VALIDATION-004 — Curved closed excluded-core 3-D Aharonov–Bohm instrument test\n\n'+
      text.replace('\n','  \n')+
      '\n\n## Interpretation\n\nThis validates a three-dimensional multiply connected lattice-gauge solver around a closed curved excluded core with externally imposed holonomy. It does not show that a rope carries flux, fixes its strength, or generates the phase dynamically.\n')
    return summary
