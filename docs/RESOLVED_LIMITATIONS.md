# Resolved Limitations -- the permanent record

Limitations that have been repaired are MOVED here, never deleted:
the kept history is part of the corpus's credential. Each entry
retains its full text as it last stood in KNOWN_LIMITATIONS.md,
with the resolving claims named.

## THE PHOTON SECTOR AT HIGH ENERGY -- RESOLVED (moved from KNOWN_LIMITATIONS at v3.23.1, 2026-08-12; resolving claims FND-083..091)
...
**STATUS (2026-08-12, end of session, FND-083..090).** The repair is no
longer merely a candidate. GRANT-SUBSTRUCTURE-TIGHT holds; the winding
structure is DERIVED (FND-088/090: hierarchy depth two by theorem, both
pitch angles in closed form, the solution unique up to level relabeling,
the Lorentz floors selecting the physical root); the 3D two-polarization
check PASSES at the derived angles on a triply-controlled instrument
(FND-089); and FND-REL-002's forced Lorentz-invariant, isotropic wave
sector is RE-DERIVED on wound carriers at Derived grade, homogenized
scope (FND-090) -- the grant's return-to-adjudication trigger was
delivered, not fired. The conviction recorded above stands as the honest
history of STRAIGHT fine strands; the adopted wound medium removes it.

**What remains owed, disclosed:** the bending-cost pricing (kb,
unscaled); the two absolute-scale parameters (m, n_sub -- the class
where the corpus's standing position locates the irreducible bottom);
the discrete O((ka)^2) question, which is FND-REL-004's exactly as for
the coarse medium; and the predictions-paper sync at the next release
cut. The soft falsifier stays armed: any confirmed photon above
m x 1.4 PeV forces m upward.
