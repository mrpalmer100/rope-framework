...
LOCK LINE (6d KOIDE-WEINBERG) 2026-09-05 on the author's word ("make
progress on other items while Casimir runs"): locked before any
sensitivity is computed. Q2 comparison point stands at 0.23122 (the
registered comparison), the low-Q^2 value displayed only. 6a/6b/6c
remain DRAFT, not locked.

LOCK LINE (6a ANTI-ALIGNED) 2026-09-05 on the author's word ("take the
next brick"). Q1 driver benchmarks/foundations/antialigned_q1.py: the
stage-2 Leg 0 protocol (a2-pinned solve at A2 = 0.02 R2 = the waypoint-1
amplitude, om2 FREE, two seeds per sector) on the 144x36 chart with the
credentialed instrument, at 3/2 (v1/v2 reproduction), 4/3, 5/3, 5/4.
The aligned roots at 4/3, 5/3, 5/4 are already held by the stage-1
waypoint-1 members (2.148, 4.234, 1.616 at matched amplitude); the anti-
aligned solves are new. Runs locally after CASIMIR-PLATE. Q2 (Lyapunov-
Schmidt on the 2D kernel) and D3 follow Q1's verdict.

LOCK LINE (6c METALLIC) 2026-09-05 on the author's word ("let's go"),
before any channel is computed.

LOCK LINE (6b AXIS-MEANING) 2026-09-05 on the author's word ("lay the
next brick"), before any point of the s-ladder exists. The Q1 external
bounds are fixed at lock by NAME only (Hughes-Drever-class clock-
comparison limits; the SME electron c-coefficient bounds), values to be
looked up in the consequence leg. The s-ladder {1.5, 2, 3, 4} and the
s^(-3/2) line are fixed here.
AMENDMENT (6b, pre-data, 2026-09-05): ELEC-099's protocol at L = 8 cannot
hold an inclusion of half-length s = 4 (it would span the periodic box).
The s-ladder runs with the box scaled to keep the registered inclusion-
to-box ratio (L = 16 s / 3, rounded to integer strand counts: L = 8, 11,
16, 21 for s = 1.5, 2, 3, 4) and the strand point spacing fixed at the
registered L/24 = 1/3 (npts = 3L). All other protocol elements verbatim
(six axes, full relaxation 250 steps, the pre-fixed K harmonic, the
three-seed floor at each s). The s^(-3/2) line is anchored at the s = 1.5
registered fraction 0.2095. Recorded before any rung exists.
